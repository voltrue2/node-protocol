...
function unpackVal(buf, noKey) {
	...
	switch (typeIndex) {
		...
		case 9:
			return unpackString(buf, noKey);
		case 10:
			return unpackArray(buf, noKey);
		case 11:
...
function unpackArray(buf, noKey) {
	const list = [];
...
function unpackObject(buf, noKey) {
	...
	const size = buf.readUInt32BE(offset);
	offset += INT32LEN;
	// extract the Buffer for the val
	var _buf = Buffer.alloc(size);
	buf.copy(_buf, 0, offset, offset + size);
	while (_buf.length) {
...
function packString(key, val) {
	const list = [];
	packKey(list, key, 'string');
	const valBytes = Buffer.from(val);
	const valByteSize = Buffer.alloc(INT32LEN);
	valByteSize.writeUInt32BE(valBytes.length);
...
function unpackString(buf, noKey) {
	const unpackedKey = unpackKey(buf, noKey);
	const typeName = unpackedKey.typeName;
	const key = unpackedKey.key;
	var offset = unpackedKey.offset;
