...
function packVal(key, val) {
	...
	switch (type) {
		...
		case 'boolean':
			if (val === true) {
				return packTrue(key, val);
			}
			return packFalse(key, val);
		case 'number':
			if (val < 0) {
				// signed
				if (val % 1 !== 0) {
...
function unpackVal(buf, noKey) {
	...
	switch (typeIndex) {
		...
		case 8:
			return unpackTrue(buf, noKey);
		case 9:
			return unpackFalse(buf, noKey);
		case 10:
		...
		case 13:
			return unpackObject8(buf, noKey);
		case 14:
			return unpackObject16(buf, noKey);
		case 15:
			return unpackObject32(buf, noKey);
		case 16:
...
function unpackObject8(buf, noKey) {
	const obj = {};
	const unpackedKey = unpackKey(buf, noKey);
	const typeName = unpackedKey.typeName;
	const key = unpackedKey.key;
...
function unpackObject16(buf, noKey) {
	const obj = {};
	const unpackedKey = unpackKey(buf, noKey);
	const typeName = unpackedKey.typeName;
	const key = unpackedKey.key;
...
function unpackObject32(buf, noKey) {
	const obj = {};
	const unpackedKey = unpackKey(buf, noKey);
	const typeName = unpackedKey.typeName;
	const key = unpackedKey.key;
...
function packTrue(key, val) {
	const list = [];
	packKey(list, key, 'true');
	return Buffer.concat(list);
}

function unpackTrue(buf, noKey) {
	const unpackedKey = unpackKey(buf, noKey);
	const typeName = unpackedKey.typeName;
	const key = unpackedKey.key;
	const offset = unpackedKey.offset;
	return {
		type: typeName,
		key: key,
		value: true,
...
function packFalse(key, val) {
	const list = [];
	packKey(list, key, 'false');
	return Buffer.concat(list);
}

function unpackFalse(buf, noKey) {
	const unpackedKey = unpackKey(buf, noKey);
	const typeName = unpackedKey.typeName;
	const key = unpackedKey.key;
	const offset = unpackedKey.offset;
